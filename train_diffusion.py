...
parser.add_argument("--emb_model_path", type=str, default="./pretrained/test.pt")
...
def main(args):
    ...
    model.accelerator = DummyAccelerator(device = "cuda:" + str(args.gpu) if args.gpu>=0 else "cpu") #if args.use_accelerator else Accelerator()
    ...
    if args.dataset_type == "waveform":
        ...
        def collate_fn(L):
            ...
            x_diff = torch.stack([xc[..., i:i + x_length] for i, xc in zip(i0, x)])
                        
            i1 = np.random.randint(0, x.shape[-1] - x_length, x.shape[0])
            x_toz = torch.stack([xc[..., i:i + x_length] for i, xc in zip(i1, x)])
        ...
        def collate_fn(L):
            ...
            i0 = np.random.randint(0, pr.shape[-1] - x_length//ae_ratio, x.shape[0])
            x_diff = torch.stack([xc[..., i*ae_ratio:i*ae_ratio + x_length] for i, xc in zip(i0, x)])
            pr = torch.stack([xc[..., i:i + x_length//ae_ratio] for i, xc in zip(i0, pr)])
