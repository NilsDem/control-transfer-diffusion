...
flags.DEFINE_integer('ae_ratio',
                     512,
                     help='Compression ratio of the AutoEncoder - required for processing midi files into the correct piano roll shape')
...
def main(dummy):
    ...
    emb_model = None if FLAGS.emb_model_path is None else torch.jit.load(FLAGS.emb_model_path).to(FLAGS.device)
    ...
    for i, (file, metadata) in enumerate(zip(tqdm(audios), metadatas)):
        ...
        if len(audio) % FLAGS.num_signal != 0:
            audio = audio[..., :FLAGS.num_signal * (length // FLAGS.num_signal)]
        ...
        for j, chunk in enumerate(chunks):
            ...
            midis.append(midi)
            chunks_buffer.append(torch.from_numpy(chunk.reshape(1,
                                                                FLAGS.num_signal)))
            metadatas_buffer.append(metadata)
            
            
            if len(chunks_buffer) == FLAGS.batch_size or (j == len(chunks) - 1
                                                    and i == len(audios) - 1):
                ...
                for array, curz, midi, cur_metadata in zip(
                    ...
                    if midi is not None:
                        ...
                        pr = midi.get_piano_roll(times=np.linspace(0, FLAGS.num_signal / FLAGS.sample_rate, FLAGS.num_signal //
                                               FLAGS.ae_ratio))
