...
def main(args):
    ...
    multi_band_distances = [(5., spectral_distance_multiband)] if pqmf_bands>1 else []
    ...
    print(
        torchinfo.summary(autoencoder.encoder,
                        verbose=1,
                        input_size=(1, pqmf_bands,
                                    x.shape[-1] // pqmf_bands)))
    print(
        torchinfo.summary(autoencoder.decoder,
                        verbose=1,
                        input_size=(1, z.shape[1], z.shape[-1])))
    torchinfo.summary(discriminator, input_size=(1, 1, x.shape[-1]))
    ...
    def collate_fn(x):
        ...
        x = [
            l[..., i0:i0 + num_signal]
            for l, i0 in zip(x, torch.randint(x[0].shape[-1] - num_signal, (len(x), )))
        ]
        x = np.stack(x)
    ...
    trainer = Trainer(autoencoder,
                    ...
                    max_steps=max_steps,
                    warmup_steps=warmup_steps, freeze_encoder_step=freeze_encoder_step,
                    rebuild_cache_steps = rebuild_cache_steps)
    ...
    trainer.fit(dataset, dataloader,
                    validloader,
                    tensorboard="./autoencoder/runs/" + model_name, device = "cuda:"+ str(args.gpu) if args.gpu >= 0 else "cpu")
